...
let run = exports.run = (() => {
  var _ref8 = (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* (config, reporter, flags, args) {
    ...
    if (args.length) {
      ...
      for (const arg of args) {
        if (matchedArgs.indexOf(arg) < 0) {
          throw new (_errors || _load_errors()).MessageError(reporter.lang('unknownPackage', arg));
        }
      }
    } else {
    ...
    var _ref11 = yield buildTree(install.resolver, install.linker, filteredPatterns);
